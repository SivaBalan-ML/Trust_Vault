...
import { Badge, EmptyState, Field, Notice, Panel, BtnDanger, BtnPrimary, Input, Textarea } from './ui'
import { BadgeCheckIcon, CodeIcon, ShieldIcon } from './icons'
        ...
        {creds.length === 0 ? (
          ...
          <div className="space-y-3">
            {creds.map((c) => (
              <div
                key={c.id}
                className="flex items-start justify-between gap-3 rounded-xl border border-ink-800 bg-ink-950/40 p-4 transition hover:border-ink-700"
              >
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5 font-mono text-[10px] text-slate-600">
                ...
                {isIssuer && c.status === 'active' && (
                  <BtnDanger className="shrink-0 !px-3 !py-1.5 text-xs" onClick={() => revoke(c.id)}>
                    Revoke
                  </BtnDanger>
                )}
              </div>
